...
def load_tsv(path, label_col=1, text_col=2):
...
def main(args):
    ...
    if args.valid and os.path.exists(args.valid):
        print("Loading validation data:", args.valid)
    ...
    print("Validate with:")
    print(f'python -c "import onnx; onnx.checker.check_model(onnx.load(\'{args.out_model}\'))"')


if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Train LR on LIAR dataset and export to ONNX")
    p.add_argument("--train", default="../Data/liar_dataset/train.tsv")
    p.add_argument("--valid", default="../Data/liar_dataset/valid.tsv")
    p.add_argument("--out-model", default="models/lr_pipeline.onnx")
    p.add_argument("--label-col", type=int, default=1, help="index of label column in TSV (0-based)")
    p.add_argument("--text-col", type=int, default=2, help="index of text/statement column in TSV (0-based)")
    args = p.parse_args()
    main(args)
